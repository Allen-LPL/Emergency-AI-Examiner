...
上传视频 -> FastAPI接收 -> 存储到磁盘 -> 创建Exam记录
         ...
         -> AI Pipeline处理:
            ...
            9. 关键词匹配 (KeywordMatcher)
            10. 多模态事件融合 (EventMerger)
            11. 评分引擎 (ScoringEngine)
            12. 报告生成 (ReportGenerator)
...
- **音频模块**: FunASR中文语音识别 + Silero-VAD + pyannote说话人分离
- **融合模块**: 多模态事件合并 + 统一时间轴
- **评分模块**: 基于规则的评分引擎，支持6个考核阶段+客观评分
